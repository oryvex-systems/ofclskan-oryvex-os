...
function productCard(p){return `<article class="card product-card" data-open-product="${p.id}" tabindex="0"><div class="photo">${p.image_url?`<img src="${esc(p.image_url)}" alt="${esc(p.name)}" onerror="this.style.display='none';this.nextElementSibling.style.display='inline-block'"><span class="material-symbols-outlined fallback-icon" style="display:none;font-size:48px">${iconFor(p.name)}</span>`:`<span class="material-symbols-outlined" style="font-size:48px">${iconFor(p.name)}</span>`}</div><div class="cardbody"><button class="add" data-product-id="${p.id}" aria-label="Sepete ekle">+</button><h3>${esc(p.name)}</h3><span class="price">${money(p.price)}</span><div class="cardhint">Detayları gör</div></div></article>`}
...
function bindProducts(){
 document.querySelectorAll('[data-product-id]').forEach(b=>b.onclick=e=>{e.stopPropagation();const p=products.find(x=>x.id===b.dataset.productId);if(!p)return;cart.push(p);saveCart();home()});
 document.querySelectorAll('[data-open-product]').forEach(card=>{const open=()=>productDetail(card.dataset.openProduct);card.onclick=e=>{if(!e.target.closest('[data-product-id]'))open()};card.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open()}}});
}
async function productDetail(id){
 const p=products.find(x=>x.id===id);if(!p)return home();
 shell(`<section class="detail-loading"><p>Ürün detayları yükleniyor…</p></section>`,'home');
 const {data:opts,error}=await supabase.from('product_options').select('id,name,is_required,min_select,max_select,sort_order,product_option_values(id,name,price_delta,is_default,sort_order)').eq('product_id',id).order('sort_order');
 if(error)console.error('Product options:',error);
 const options=(opts||[]).map(o=>({...o,product_option_values:(o.product_option_values||[]).sort((a,b)=>(a.sort_order||0)-(b.sort_order||0))}));
 shell(`<button class="detail-back" id="detailBack"><span class="material-symbols-outlined">arrow_back</span> Geri</button><section class="product-detail"><div class="detail-photo">${p.image_url?`<img src="${esc(p.image_url)}" alt="${esc(p.name)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="detail-fallback" style="display:none"><span class="material-symbols-outlined">${iconFor(p.name)}</span></div>`:`<div class="detail-fallback"><span class="material-symbols-outlined">${iconFor(p.name)}</span></div>`}</div><div class="detail-body"><div class="detail-title-row"><div><div class="detail-brand">${/burger/i.test(p.name)?'BURGERMY • TIKLADOY':'TIKLADOY'}</div><h1>${esc(p.name)}</h1></div><div class="detail-price">${money(p.price)}</div></div><p class="detail-desc">${esc(p.description||'Özenle hazırlanan ürün. Siparişinden önce seçeneklerini belirleyebilirsin.')}</p>${options.length?`<div class="option-groups">${options.map((o,oi)=>`<section class="option-group"><div class="option-head"><div><h3>${esc(o.name)}</h3><span>${o.is_required?'Seçim gerekli':'İsteğe bağlı'}</span></div>${o.max_select>1?`<small>En fazla ${o.max_select}</small>`:''}</div><div class="option-values">${o.product_option_values.map((v,vi)=>`<label class="option-value"><span><input type="${o.max_select===1?'radio':'checkbox'}" name="opt-${oi}" value="${v.id}" data-label="${esc(v.name)}" data-delta="${Number(v.price_delta||0)}" ${v.is_default?'checked':''}> ${esc(v.name)}</span><b>${Number(v.price_delta||0)>0?`+${money(v.price_delta)}`:'Dahil'}</b></label>`).join('')}</div></section>`).join('')}</div>`:''}<div class="detail-qty"><button id="qtyMinus">−</button><strong id="qtyValue">1</strong><button id="qtyPlus">+</button></div><button class="btn detail-add" id="detailAdd"><span>Sepete Ekle</span><b id="detailTotal">${money(p.price)}</b></button></div></section>`,'home');
 let qty=1;
 const totalEl=document.querySelector('#detailTotal');
 const calc=()=>{const delta=[...document.querySelectorAll('.option-value input:checked')].reduce((s,x)=>s+Number(x.dataset.delta||0),0);totalEl.textContent=money((Number(p.price)+delta)*qty)};
 document.querySelectorAll('.option-value input').forEach(x=>x.onchange=calc);
 document.querySelector('#qtyMinus').onclick=()=>{qty=Math.max(1,qty-1);document.querySelector('#qtyValue').textContent=qty;calc()};
 document.querySelector('#qtyPlus').onclick=()=>{qty++;document.querySelector('#qtyValue').textContent=qty;calc()};
 document.querySelector('#detailBack').onclick=()=>history.length>1?history.back():home();
 document.querySelector('#detailAdd').onclick=()=>{for(let i=0;i<qty;i++)cart.push(p);saveCart();cartPage()};
}
