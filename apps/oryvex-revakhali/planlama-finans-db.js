...
async function save(){if(!rt?.sb||!pid)return;const f=finance();const day=new Date().toISOString().slice(0,10);const row={project_id:pid,company_id:rt.company_id,payment_no:Math.max(1,Math.trunc(Number(f.payment_no)||1)),employer_earned:Number(f.employer_earned||0),subcontractor_net:Number(f.subcontractor_net||0),collected_amount:Number(f.collected_amount||0),paid_amount:Number(f.paid_amount||0),gross_margin:Number(f.gross_margin||0),cash_difference:Number(f.cash_difference||0),snapshot_date:day};const q=await rt.sb.from("santiye_finance_snapshots").select("id").eq("project_id",pid).eq("company_id",rt.company_id).eq("payment_no",row.payment_no).limit(1);if(q.error){console.warn("ORYVEX Finans lookup",q.error);return;}const r=q.data?.[0]?await rt.sb.from("santiye_finance_snapshots").update(row).eq("id",q.data[0].id):await rt.sb.from("santiye_finance_snapshots").insert(row);if(r.error)console.warn("ORYVEX Finans save",r.error);else console.info("ORYVEX Finans snapshot kaydedildi",day);}
