(() => {
  ...
  async function callEdge(name, init = {}, token = '') {
    return nativeFetch(edge(name), {
      ...init,
      headers: { ...baseHeaders(token), ...(init.headers || {}) },
    });
  }
  ...
  window.fetch = async function(input, init = {}) {
    ...
    try {
      ...
      if (url === '/api/payments/paytr' && method === 'POST') {
        ...
        const body = JSON.parse(init.body || '{}');
        const response = await callEdge('paytr-create-token', {
          method: 'POST',
          body: JSON.stringify({ orderId: body.orderId, userName: body.userName, userAddress: body.userAddress, userPhone: body.userPhone }),
        }, token);
