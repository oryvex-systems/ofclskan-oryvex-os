...
export default function BurgerMyApp() {
  ...
  async function placeOrder() {
    ...
    try {
      ...
      const payResponse = await fetch("/api/payments/paytr", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ orderId: order.orderId, userAddress: delivery === "Gel-Al" ? selectedBranch.address : address }),
      });
  ...
  return <div className={`app-shell ${authMode ? "auth-mode" : ""}`}>
    ...
    <main className="main-canvas">
      ...
      {screen === "profile" && <section className="inner-page"><PageTop title="Profil" back={() => go("home")} /><div className="form-card"><h2>Hesabım</h2><p>{userEmail || "BURGERMY müşterisi"}</p><p>Giriş için e-posta veya Google hesabı kullanılır.</p><button className="secondary-btn wide" onClick={logout}>Çıkış Yap</button></div></section>}
    </main>
