...
const defaultOps: Operations = { deliveryEnabled: true, pickupEnabled: true, onlineCardEnabled: true, doorPosEnabled: false, cashEnabled: false, minimumOrder: 150, freeDeliveryThreshold: null };
...
export default function BurgerMyApp() {
  ...
  const [payment, setPayment] = useState<"paytr" | "door_pos">("paytr");
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (catalog.operations) setOperations(catalog.operations);
  ...
  async function placeOrder() {
    ...
    try {
      ...
      if (payment === "door_pos") { setCart([]); go("success"); return; }
